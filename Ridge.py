...
def ridge(file_num = 10, impfeat = 100, bootstrap_num = 10):
   
    logging.basicConfig(level=logging.DEBUG)
    
    ridge_delta = open('results/ridge_delta.txt', 'wb')
    ...
    for a,alpha in enumerate(ALPHA_VALUES):
        ...
        for k in range(file_num):
            ...
            non_zero_features = np.sum(ridge.coef_ != 0)
            features_selected += non_zero_features
    
            max_position, computed_ranking = getrankings(ridge.coef_, impfeat)
            max_positions.append(max_position)
                
            rcor = scipy.stats.spearmanr(computed_ranking, np.array(range(impfeat)))[0]
            rcors.append(rcor)
                                    
            l = np.argsort(ridge.coef_)[-non_zero_features:]  
            ...
            #plot weights
            plot_weights_no_boot(alpha, rho, k, bootstrap_num, features, ridge.coef_)
            fname = 'results/weights_' + str(alpha) + '_' + str(rho) + '_' + str(k) + '.txt'
            np.savetxt(fname, ridge.coef_)                
            add_to_zip(fname)
            os.remove(fname)
    ...
    ridge_delta.write(delta)
    ridge_delta.close()
    


if __name__ == "__main__":
   ridge(file_num = 10, impfeat = 100, bootstrap_num = 10)
